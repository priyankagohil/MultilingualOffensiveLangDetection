...
from data_utils import OLID, OLIDUniformSample, OLIDSOLID, SOLID
...
olid_trainDataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True)

dataset = SOLID(dataroot)
solid_trainDataloader = DataLoader(dataset, batch_size=batch_size, shuffle=True)
...
for epoch_i in range(epochs):
  ...
  train_loss, train_acc = train_model(solid_trainDataloader)
  logprint(f"SOLID Train loss: {train_loss} Train Acc: {train_acc}\n")
  ...
  logprint(f"SOLID overall Val Loss: {val_loss}: Val Acc: {val_acc}\n")
  logprint(f'SOLID overall Val Precision:{precision} Recall: {recall} F1: {f1}\n')

  logprint('========================\n')
  logprint('Training on OLID...\n')
  logprint('========================\n')

  train_loss, train_acc = train_model(olid_trainDataloader)
  logprint(f"OLID Train loss: {train_loss} Train Acc: {train_acc}\n")

  for test_lang in langs:
      precision, recall, f1, val_loss, val_acc = validate_model(test_loaders[test_lang])
    #   logprint(f"Val Loss: {val_loss}: Val Acc: {val_acc}\n")
      logprint(f'OLID Lang: {test_lang} Val Precision:{precision} Recall: {recall} F1: {f1}\n')


  precision, recall, f1, val_loss, val_acc = validate_model(testDataloader)

  logprint(f"OLID overall Val Loss: {val_loss}: Val Acc: {val_acc}\n")
  logprint(f'OLID overall Val Precision:{precision} Recall: {recall} F1: {f1}\n')
  ...
  if f1 > best_val_f1:
    ...
    save_model_path = os.path.join(args.logs_dir, 'xlm_iterative.pth')
